...
{% macro narrative() -%}
{% if KPI_CASE == 'World' %}
  In the last <b>{{ lastdays }} days</b>, <b class="color-neg">{{ '{0:,.0f}'.format(D['Cases 3(+)']) }}</b> new Coronavirus cases have been reported worldwide.
  ...
  <table class="table" style="width:575px;">
    ...
    {% for index, row in table.iterrows() %}
      <tr>
        ...
        <td class="change neg">(<b>{{ '{0:+,.0f}'.format(row['Cases 3(+)']) }}</b>)</td>
        <td class="pl1">{{ '{0:,.0f}'.format(row['Deaths']) }}</td>
        <td class="change neg">(<b>{{ '{0:+,.0f}'.format(row['Deaths 3(+)']) }}</b>)</td>
